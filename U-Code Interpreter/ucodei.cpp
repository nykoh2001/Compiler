...
void Label::findLabel(char label[], int instr)
{
	...
	for (index = 0; (index <= labelCnt) &&
									strcmp(label, labelTable[index].labelName);
			 index++)
		;
	if (index > labelCnt)
	{ // not found
		...
		ptr = new fixUpList;
		if (ptr == NULL)
			errmsg("findLabel()", "Out of memory -- new");
	...
	{ // found
		...
		if (ptr)
			...
			instrBuf[instr].value1 = labelTable[index].address;
	}
}
...
void Assemble::getLabel()
{
	...
	while (isspace(lineBuffer[bufIndex]))
		bufIndex++;
...
int Assemble::getOpcode()
{
	...
	for (index = notop; index < none; index++)
		if (!strcmp(mnemonic, opcodeName[index]))
			break;

	if (index == none)
		errmsg("Illegal opcode", mnemonic);
	return index;
}
...
void Interpret::execute(int startAddr)
{
	...
	while (pc >= 0)
	{
		...
		if (executable[instrBuf[pc].opcode])
			exeCount++;
...
int main(int argc, char *argv[])
{
	...
	if (argc != 3)
		errmsg("main()", "Wrong number of arguments");

	inputFile.open(argv[1], ios::in);
	if (!inputFile)
		errmsg("cannot open input file", argv[1]);
